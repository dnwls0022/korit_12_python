...
#1.전체 후보자의 수를 사용자로부터 입력받으시오.
count = int(input("후보자 수를 입력하시오>>> "))
#1-1 후보자 리스트 생성
candidates = []

#2 .for 반복문을 사용하여 후보자 수만큼 후보자들의 이름을 입력받고, candidates 리스트에 저장하시오
for i in range(1,count + 1):
    num = (input(f"{i}번째 후보자의 이름를 입력하시오 >>> "))
    candidates.append(num)
#3. 각 후보자의 투표 수를 저장할 딕셔너리(votes)를 생성하고, candidates 리스트의 이름을 키로, 초기 투표 수(0)를 값으로 설정하시오.
candidate = {
    'candidates': candidates
}
#각 후보자의 투표 수를 저장할 딕셔너리(votes)
dic_votes = {}
#  초기 투표 수(0)를 값
Initial_Votes =0
#4. 전체투표횟수
Total_number_of_votes = input('전체 투표 횟수를 입력하시오 >>>')
#5. for 반복문을 사용하여 투표 횟수만큼 사용자로부터 각 투표 결과를 입력받으시오. 입력은 후보자의 순서 번호(1, 2, 3...)로 받으시오.
for i in range(1, int(Total_number_of_votes) + 1):
    num = (input(f"{i}번째 투표 (1: 김일, 2: 김이, 3: 김삼) >>>  "))
    candidates.append(num)
#6. 입력받은 투표 결과에 따라 해당 후보자의 투표 수를 1씩 증가시키시오.
    Total_number_of_votes[candidates.index(int(Total_number_of_votes))] += 1
#7. 최종적으로 딕셔너리에 저장된 각 후보자의 투표 수를 출력하시오
print(f"현재{candidates} : {Total_number_of_votes}표")
